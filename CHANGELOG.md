...
- **Remove repos from the list** — right-click any repo and choose **Remove from List** to hide it from the dashboard. The repo's folder is never touched on disk, and the removal survives rescans and app restarts. Restore hidden repos from the toolbar's **Folders ▸ Hidden Repositories** menu — individually, or all at once with **Show All**; the submenu appears only while something is hidden.

### Fixed

- The identity footer is now opaque, so repo rows scrolling beneath it no longer show through.
- The light-mode sidebar grey is darker (`#ECECEE`), giving the pane clearer separation from the detail view.

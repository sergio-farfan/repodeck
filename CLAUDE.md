...
RepoDeck ships local releases (no CI) while the repo is private.
...
**Deviation from the family convention (documented):** alttab publishes releases via a `release.yml` GitHub Actions workflow on `macos-*` runners triggered by `v*` tags. RepoDeck does not have this workflow yet — macOS GitHub-hosted runners bill at 10x the Linux rate, which isn't worth it for a private repo with no external consumers. Once this repo goes public, copy alttab's `.github/workflows/release.yml` and wire `Scripts/make-dmg.sh` in as its packaging step.

## Notes

- Repo-local git identity is `sergio.farfan@gmail.com` (not the global Accenture identity).
- Commits are conventional (`feat:`, `fix:`, `docs:`, `chore:`, ...); no AI attribution in commit messages or generated docs.
